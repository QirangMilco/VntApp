...
class _DashboardPageState extends State<DashboardPage> {
  ...
  void _updateStats() {
    ...
    for (var entry in allVnts.entries) {
      ...
      if (!vntBox.isClosed()) {
        ...
        if (config != null) {
          ...
          // 获取协议类型
          protocol = config.protocol.isNotEmpty
        ...
        if (virtualGateway.isNotEmpty) {
          ...
          } else if (gatewayRoute != null &&
              (gatewayRoute.rt == 0 || gatewayRoute.rt == 9999)) {
          ...
          // 仅在拿到有效网关路由结果时记录，避免 route==null 被误记为持续丢包
          if (gatewayRoute != null && shouldRecord) {
    ...
    // 计算网关探测失败率：(不通次数 / 总次数) * 100
    double packetLoss = 0.0;
    if (_gatewayConnectivityHistory.isNotEmpty) {
      int failedCount =
          _gatewayConnectivityHistory.where((connected) => !connected).length;
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            BoxShadow(
              color:
                  (hasConnection ? primaryColor : Colors.grey).withOpacity(0.3),
              blurRadius: 20,
              ...
              GestureDetector(
                ...
                child: IconButton(
                  ...
                  icon: Icon(Icons.power_settings_new,
                      color: Colors.white, size: context.iconSize(28)),
                  tooltip: '断开连接',
                ),
              )
            else
  ...
  void _showTrafficStatisticsDialog(bool isDark) {
    ...
    for (var entry in allVnts.entries) {
      final vntBox = entry.value;
      if (!vntBox.isClosed()) {
        deviceTrafficList = vntBox.vntApi?.streamAll() ?? const [];
        break; // 只取第一个连接的数据
      }
    }
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildDeviceInfoItem(isDark, '本地 IPv4',
                          config.localIpv4.isEmpty ? '自动识别' : config.localIpv4),
